...
export type SessionSwitchPayload = {
  session: { id: number; productId: number; roomId: string; status: SessionStatus }
  product: { id: number; name: string; coverUrl: string; description?: string }
  snapshot: SessionSnapshot
}

export type CommentNewPayload = {
  comment: {
    id: number
    roomId: string
    userId: number
