...
func (r *SocialRepo) InsertComment(ctx context.Context, roomID string, userID uint64, content string, liveRoomID *uint64) (uint64, error) {
	var lr sql.NullInt64
	if liveRoomID != nil {
		lr = sql.NullInt64{Int64: int64(*liveRoomID), Valid: true}
	}
	const q = `INSERT INTO room_comments (room_id, live_room_id, user_id, content) VALUES (?, ?, ?, ?)`
	res, err := r.db.ExecContext(ctx, q, roomID, lr, userID, content)
	if err != nil {
		return 0, err
	}
...
func (r *SocialRepo) GetComment(ctx context.Context, id uint64) (*domain.RoomComment, error) {
	...
	return scanSocialComment(r.db.QueryRowContext(ctx, q, id))
}
...
func scanSocialComment(row interface {
	Scan(dest ...any) error
}) (*domain.RoomComment, error) {
	var c domain.RoomComment
	var roomID sql.NullString
	var hidden int
	err := row.Scan(&c.ID, &roomID, &c.UserID, &c.Nickname, &c.Avatar, &c.Content, &hidden, &c.CreatedAt)
