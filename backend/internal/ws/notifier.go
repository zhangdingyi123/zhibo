...
type CommentNewPayload struct {
	Comment service.CommentView `json:"comment"`
}

func (n *Notifier) OnSessionSwitch(ctx context.Context, roomID string, session *domain.AuctionSession, product *domain.Product) {
	if n == nil || n.hub == nil || session == nil || product == nil {
		return
	}
	snap := service.BuildSnapshot(session, time.Now())
	n.hub.Publish(roomID, EventSessionSwitch, SessionSwitchPayload{
		Session:  *session,
		Product:  *product,
		Snapshot: snap,
	})
...
func (n *Notifier) OnCommentNew(ctx context.Context, roomID string, comment service.CommentView) {
	if n == nil || n.hub == nil {
		return
	}
	n.hub.Publish(roomID, EventCommentNew, CommentNewPayload{Comment: comment})
}
