...
type CreateSessionInput struct {
	...
	AnchorID         uint64
	LiveRoomID       *uint64
	RoomID           string // 非空时直接使用（直播间共享 room_id）
...
func (r *SessionRepo) Create(ctx context.Context, in CreateSessionInput) (*domain.AuctionSession, error) {
	...
	const q = `INSERT INTO auction_sessions (
		product_id, anchor_id, live_room_id, room_id, status,
	...
	res, err := r.db.ExecContext(ctx, q,
		in.ProductID, in.AnchorID, liveRoomID, placeholderRoom,
...
func (r *SessionRepo) GetByRoomID(ctx context.Context, roomID string) (*domain.AuctionSession, error) {
	// 直播间：按 current_session_id 或首个待播场次解析
	var liveRoomID, currentSession sql.NullInt64
	err := r.db.QueryRowContext(ctx,
		`SELECT id, current_session_id FROM live_rooms WHERE room_id = ?`, roomID).
		Scan(&liveRoomID, &currentSession)
	if err == nil {
		if currentSession.Valid {
			return r.GetByID(ctx, uint64(currentSession.Int64))
		}
		var firstSession sql.NullInt64
		_ = r.db.QueryRowContext(ctx,
			`SELECT session_id FROM live_room_items
			 WHERE live_room_id = ? AND session_id IS NOT NULL AND status IN ('queued', 'on_air')
			 ORDER BY sort_order ASC LIMIT 1`, liveRoomID.Int64).Scan(&firstSession)
		if firstSession.Valid {
			return r.GetByID(ctx, uint64(firstSession.Int64))
		}
		return nil, domain.ErrNotFound
	}
	if !errors.Is(err, sql.ErrNoRows) {
		return nil, fmt.Errorf("resolve live room: %w", err)
	}
	...
	const q = `SELECT id, product_id, anchor_id, room_id, status,
		...
		ORDER BY CASE status WHEN 'running' THEN 0 WHEN 'pending' THEN 1 ELSE 2 END, id DESC
		LIMIT 1`
